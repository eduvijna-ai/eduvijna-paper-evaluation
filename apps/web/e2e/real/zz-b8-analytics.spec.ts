...
test.describe("B8 live analytics + mastery evidence (real API)", () => {
  test("two published submissions drive live assessment and student analytics", async ({
    ...
    await expect(page.getByTestId("learning-not-live-boundary")).toHaveCount(0);
    await expect(page.getByRole("link", { name: /Open learning plan/i })).toBeVisible();
    ...
    await expect(page.getByTestId("learning-materialization-status")).toBeVisible();
  });
});
