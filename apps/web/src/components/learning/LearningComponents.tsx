...
export function ErrorDistribution({
  ...
  return (
    <div data-testid="error-distribution" className="space-y-2">
      {items.map((item) => (
        <div key={item.code} className="grid grid-cols-[140px_1fr_32px] items-center gap-2">
          ...
          <span className="text-xs tabular-nums text-slate-600">{item.count}</span>
        </div>
      ))}
    </div>
  );
...
const STEP_LABELS: Record<LearningPathStepItem["kind"], string> = {
