...
- Original uploads **write-once** to S3/MinIO with content hash (`source_content_hash`).
...
Fields: `actor_id`, `actor_type`, `event_type`, `resource_type`, `resource_id`, `correlation_id`, `payload`, `created_at`.
