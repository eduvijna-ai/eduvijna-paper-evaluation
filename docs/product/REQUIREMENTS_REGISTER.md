...
10. **APP-008 / B16 implementation:** PEV-044–046 remain `FUTURE_ENTERPRISE` in Release State (planning gate). They are implemented on `develop` under B16 / Issue #60 per APP-008 (horizontal question grading pools, configurable moderation, formal grievance re-evaluation with versioned runs/results; no double-count of superseded publications; no `main` promotion). PEV-048–051 and PEV-054–057 remain deferred.
