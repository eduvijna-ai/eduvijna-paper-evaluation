...
|------|----------|----------|--------|-------|
| 2026-09-18 | **Approve Multi-Subject & Multilingual Understanding release promotion.** Authorizes governed promotion of the independently accepted APP-017 / B20 implementation (PEV-056–057) from `develop` to `main`. Release/governance only; no new product implementation; PEV-056–057 remain formally `FUTURE_ENTERPRISE`; Alembic head remains `20260918_0022`; exact-tree snapshot mandatory because main/develop histories intentionally diverge; no direct develop→main history merge; no merge-back from main to develop; no production AI provider/model deployment, model training/fine-tuning, open-web enrichment, dependency expansion, or unrelated scope. Founder authorization: “ok proceed”. | Founder / Product Architect | APPROVED | APP-018. Milestone: Multi-Subject & Multilingual Understanding Release. Issue #109. Accepted develop: `3e6bc6cf085bd95d7298b270ffb50b162b57aaf3` (tree `934611d9c285019ddb2de93a0b4bf7b6c59165dd`). Expected main until promotion: `5ce220dd87c1c038d9bda18d175a77d8695179ff` (tree `04ca96b02928c8a7efba423d6dc77b8e9f7982b9`). B20 accepted via Issue #107 / PR #108 after CI run `35360841332` (all six authoritative jobs SUCCESS). |
...
## Detailed Entry — APP-018

### APP-018 — Multi-Subject & Multilingual Understanding Release Promotion

| Field | Value |
|-------|-------|
| **Date** | 2026-09-18 |
| **Decision** | Founder approves governed release promotion of the independently accepted APP-017 / B20 Multi-Subject & Multilingual Understanding implementation from `develop` to `main`. Scope is the already accepted PEV-056 Multi-Subject Expansion and PEV-057 Multilingual Handwriting implementation. Founder authorization recorded in ChatGPT on 2026-09-18: **“ok proceed.”** |
| **Approver** | Founder / Product Architect |
| **Status** | APPROVED |
| **Notes** | Tracking Issue #109. Accepted release-source `develop`: `3e6bc6cf085bd95d7298b270ffb50b162b57aaf3` (tree `934611d9c285019ddb2de93a0b4bf7b6c59165dd`). Expected unchanged `main` until promotion: `5ce220dd87c1c038d9bda18d175a77d8695179ff` (tree `04ca96b02928c8a7efba423d6dc77b8e9f7982b9`). Accepted B20 feature head: `5a37bf6684b224042093f1aef14b54d44182c88e`; authoritative pre-merge CI run `35360841332` had all six required jobs SUCCESS; B20 squash: `3e6bc6cf085bd95d7298b270ffb50b162b57aaf3`. Alembic head remains `20260918_0022` with `down_revision = 20260917_0021`. |

#### Release invariants

1. This is release/governance only: no new backend/frontend feature, schema redesign, migration, dependency expansion, requirement reclassification, or infrastructure architecture change.
2. PEV-056 and PEV-057 remain formally `FUTURE_ENTERPRISE`; implementation/release does not change their register classification.
3. `Assessment.subject_node_id` remains the canonical subject anchor; no parallel Subject database or independent grading pipeline is introduced.
4. Unknown/unconfigured subject profiles continue to fail closed; Mathematics-only symbolic verification remains Mathematics-scoped.
5. Original-language transcription remains authoritative evidence; translation/transliteration remains linked derived evidence and may not replace original text.
6. Review-required/unsupported language paths remain fail-closed and evaluator confirmation remains permission-governed.
7. Existing evaluation-ledger, human approval, publication, moderation, grievance, supersession, analytics, mastery, and audit authority remains unchanged.
8. No production AI provider/model deployment, model training/fine-tuning, training-data export, open-web enrichment/search, or live third-party credential rollout is authorized by this release decision.
9. Main/develop histories intentionally diverge. Do **not** directly merge `develop` into `main`, hand-resolve conflicts, or merge `main` back into `develop`.
10. Promotion must use the established conflict-safe **exact-tree snapshot** pattern: the release snapshot is parented from the exact current `main` SHA while its Git tree equals the independently accepted release-source `develop` tree exactly.
11. The APP-018 governance PR is documentation-only and may modify only `docs/FOUNDER_APPROVAL_LOG.md`.
12. All six authoritative CI jobs must succeed on the exact final APP-018 governance head before governance merge to `develop`.
13. After independent ChatGPT acceptance of the governance merge, all six authoritative CI jobs must succeed on the exact release snapshot head before merge to `main`.
14. After promotion, all six authoritative post-main CI jobs must succeed on the exact final `main` SHA before Issue #109 may close.
15. No Dependabot incorporation, unrelated cleanup, failed-job-only acceptance rerun, or historical Git reconciliation is permitted in either governance or release snapshot PR.
...
## Detailed Entry — APP-016

### APP-016 — Enterprise Identity & Interoperability Release Promotion

| Field | Value |
|-------|-------|
| **Date** | 2026-09-18 |
| **Decision** | Founder approves governed release promotion of the independently accepted APP-015 / B19 Enterprise Identity & Interoperability implementation from `develop` to `main`. Scope is the already accepted PEV-054 Enterprise SSO & SCIM and PEV-055 LMS / SIS / LTI interoperability implementation, including machine API and outbound webhooks. Founder authorization recorded in ChatGPT on 2026-09-18: **“proceed.”** |
| **Approver** | Founder / Product Architect |
| **Status** | APPROVED |
| **Notes** | Tracking Issue #102. Accepted release-source `develop`: `6f80d6d43bb0b9f8e96288147cd35c17f93042f1` (tree `389cb0cf4fc2c23c266d5b2701f302ad27af00ca`). Expected unchanged `main` until promotion: `8f488b12ade2a6dffd60c8df5dc4c5fcfda9d7ce` (tree `362011e330915335e237cb4322ed45dd83940736`). Accepted B19 feature head: `d58c385c730edf56ef272d16737abeff73012008`; authoritative pre-merge CI run `35306547812` had all six required jobs SUCCESS; B19 squash: `6f80d6d43bb0b9f8e96288147cd35c17f93042f1`. Alembic head remains `20260917_0021`. |

#### Release invariants

1. This is release/governance only: no new backend/frontend feature, schema redesign, migration, dependency expansion, requirement reclassification, or infrastructure architecture change.
2. PEV-054 and PEV-055 remain formally `FUTURE_ENTERPRISE`; implementation/release does not change their register classification.
3. PEV-056 Multi-Subject Expansion and PEV-057 Multilingual Handwriting remain deferred and are not activated by APP-016.
4. No production IdP/LMS/SIS credentials, tenant configuration, or live third-party service deployment is authorized by this release decision.
5. Main/develop histories intentionally diverge. Do **not** directly merge `develop` into `main`, hand-resolve conflicts, or merge `main` back into `develop`.
6. Promotion must use the established conflict-safe **exact-tree snapshot** pattern: the release snapshot is parented from the exact current `main` SHA while its Git tree equals the independently accepted release-source `develop` tree exactly.
7. The APP-016 governance PR is documentation-only and may modify only `docs/FOUNDER_APPROVAL_LOG.md`.
8. All six authoritative CI jobs must succeed on the exact final APP-016 governance head before governance merge to `develop`.
9. After independent ChatGPT acceptance of the governance merge, all six authoritative CI jobs must succeed on the exact release snapshot head before merge to `main`.
10. After promotion, all six authoritative post-main CI jobs must succeed on the exact final `main` SHA before Issue #102 may close.
11. No Dependabot incorporation or unrelated cleanup is permitted in either governance or release snapshot PR.
